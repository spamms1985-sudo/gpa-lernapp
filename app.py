...
def student_overview(users_df: pd.DataFrame, attempts_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregates per student: last activity, totals, accuracy, fields started, list of learn fields."""
    if users_df.empty:
        return pd.DataFrame()
    if attempts_df.empty:
        out = users_df.copy()
        ...
        out["learnfields"] = ""
        return out[["user_id","display_name","class_code","attempts_total","accuracy","last_activity","fields_started","learnfields"]]
    ...
    g = a.groupby("user_id").agg(
        ...
        fields_started=("field_id", pd.Series.nunique),
    ).reset_index()
    g["accuracy"] = (g["correct_total"] / g["attempts_total"]).round(2)

    lf_list = a.groupby("user_id")["field_id"].apply(lambda s: ", ".join(sorted(set(s)))).reset_index(name="learnfields")

    out = users_df.merge(g, on="user_id", how="left").merge(lf_list, on="user_id", how="left")
...
with st.sidebar:
    ...
    # Auto-eindeutige IDs für Schüler:innen (verhindert, dass alle als "demo_schueler" in einem Konto landen)
...
with tabs[1]:
    ...
    st.markdown("### 1) Schülerübersicht (Aktivität & Lernfelder)")
    ...
    if ov.empty:
        ...
        st.dataframe(
            ov_show[["ID","Name","Aufgaben","Trefferquote","Letzte Aktivität","Lernfelder gestartet","Lernfelder"]],
            use_container_width=True, hide_index=True
        )



    st.markdown("### 2) Übersicht: Wo liegen die größten Schwierigkeiten?")
    ...
    st.markdown("### 3) Einzelanalyse: Fehler & Förderung pro Schüler:in")
    if class_users.empty:
        ...
        st.markdown("**Konkrete Fehler (letzte 15 falsche Antworten)**")
        wrong = s_attempts[s_attempts["correct"] == 0].head(15)
        if wrong.empty:
    ...
    st.markdown("### 4) Eigenes Material hochladen (für Schüler:innen)")

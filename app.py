...
if st.button("Weiter", key=btn_key):
    ...
    if t=="order":
        # Better UI: choose the step for each position (no confusing "1-3-2-4" text field)
        steps = list(item["steps"])
        st.caption("Wähle die richtige Reihenfolge: Position 1 → 2 → 3 → …")
        chosen_steps = []
...
if st.button("Weiter", key=btn_key):
            ...
            if len(set(chosen_steps)) != len(chosen_steps):
                ...
                return None
            # map chosen order back to indices

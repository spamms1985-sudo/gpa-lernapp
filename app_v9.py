    ...
    if not global_done:
        ...
        with c2:
            if st.button("Üben (adaptiv) starten", key="start_practice"):
                ...
                seen = set(at["item_id"].tolist()) if not at.empty else set()
                chosen = pick_adaptive(lf_items, p, field_id, seen, k=10)
                st.session_state["flow_items"]=chosen
                st.session_state["flow_i"]=0
...
with tabs[1]:
    ...
    # 4) Materials
    st.markdown("### 4) Material hochladen (für Schüler:innen)")
